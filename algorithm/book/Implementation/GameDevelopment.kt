...
class GameDevelopment {
    fun solution (userPosition : IntArray, map: Array<IntArray>):Int{
        val walkStack = mutableListOf<IntArray>()

        // 처음의 포지션을 일단 walkStack에 넣기
        walkStack.add(intArrayOf(userPosition[0], userPosition[1])) // (1,1)을 먼저 넣기
        ...
        while (true){
            when(userCompass){
                0 -> { // 북쪽을 보고 있는 경우
                    if ((map[userPos[0]][userPos[1] - 1] == 0) && !walkStack.contains(intArrayOf(userPos[0], userPos[1] - 1))){ // 탐색 시 갈수 있는 위치라면
                        // 북 -> 서쪽 바라보고 위치값 갱신
                        rotateCount = 0
                        userCompass = 3
                        userPos[0] = userPos[0]
                        ...
                        walkStack.add(intArrayOf(userPos[0], userPos[1]))
                    } else if ((map[userPos[0]][userPos[1] - 1] == 1) || walkStack.contains(intArrayOf(userPos[0], userPos[1] - 1))){
                ...
                1 -> { // 동쪽을 보고 있는 경우
                    if ((map[userPos[0] - 1][userPos[1]] == 0) && !walkStack.contains(intArrayOf(userPos[0] - 1 , userPos[1]))){ // 탐색 시 갈수 있는 위치라면
                        // 동 -> 북쪽 바라보고 위치값 갱신
                        rotateCount = 0
                        userCompass = 0
                        userPos[0] = userPos[0] - 1
                        ...
                        // 옮긴 뒤 현재 포지션을 walkStack에 넣기
                        walkStack.add(intArrayOf(userPos[0], userPos[1]))
                    } else if ((map[userPos[0] - 1][userPos[1]] == 1) || walkStack.contains(intArrayOf(userPos[0] - 1, userPos[1]))){
                        // 그 위치가 바다이거나 가본 칸인 경우 회전만 함
                        userCompass = 0
                        rotateCount++
